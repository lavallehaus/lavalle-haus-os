...
/* ============================================================================
   LAVALLE HAUS OS — GOOGLE ADS
   Manual tracker for Google Ads campaigns: spend, conversion value, ROAS, CPA.
   CRUD with session Undo/Redo. Persists through onSave (parent -> Redis under
   dbState.googleAds).
       <GoogleAds data={dbState.googleAds || []} onSave={(rows) => ...} />
   ========================================================================== */
...
const TYPES = ["Search", "Performance Max", "Shopping", "Display", "YouTube", "Demand Gen"];
const STATUSES = ["active", "paused", "testing"];
const BLANK = { name: "", type: "Search", budget: "", spend: "", revenue: "", conversions: "", clicks: "", status: "active", notes: "" };

const roasOf = (a) => { const s = num(a.spend); return s > 0 ? num(a.revenue) / s : 0; };
const cpaOf = (a) => { const cv = num(a.conversions); return cv > 0 ? num(a.spend) / cv : 0; };
...
export default function GoogleAds({ data = [], onSave }) {
  ...
  const clean = (d) => ({ ...d, budget: num(d.budget), spend: num(d.spend), revenue: num(d.revenue), conversions: num(d.conversions), clicks: num(d.clicks) });
  ...
  const form = (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px,1fr))", gap: 10 }}>
      <label style={{ gridColumn: "1 / -1" }}><div style={S.cap}>Campaign name · nombre</div><input style={S.input} value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="e.g. Brand Search - Candles" /></label>
      ...
      <Field k="revenue" label="Conv. value $ · ingresos" ph="0" />
      <Field k="conversions" label="Conversions · conversiones" ph="0" />
  ...
  return (
    <div style={S.wrap}>
      <div>
        <h1 style={S.h1}>Google Ads</h1><div style={faintEs}>Anuncios de Google</div>
        <div style={S.sub}>Track spend, conversion value, and ROAS across your Google campaigns.</div>
        <div style={faintEs}>Gasto, valor de conversión y ROAS de tus campañas de Google.</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px,1fr))", gap: 12, marginTop: 14 }}>
        {[
          ...
          { l: "Conv. value", le: "Ingresos", v: money0(totalRev), color: c.green },
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
        {rows.length === 0 && !adding && <div style={{ ...S.panel, fontStyle: "italic", color: c.sub, fontSize: 13 }}>No Google campaigns yet — add your first. · Sin campañas aún.</div>}
        {rows.map((a) => {
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    ...
                    <div style={{ fontSize: 12, color: c.sub, marginTop: 3 }}>
                      {dollars(a.spend)} spent · {dollars(a.revenue)} value{num(a.budget) ? ` · ${dollars(a.budget)}/day` : ""}
                    </div>
